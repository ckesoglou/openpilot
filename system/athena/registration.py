...
def register(show_spinner=False, register_alternate=False) -> str | None:
  """
  ...
  if not public_key and not register_alternate:
    dongle_id = UNREGISTERED_DONGLE_ID
    cloudlog.warning("missing public key")
  elif dongle_id is None or register_alternate:
    if show_spinner:
      spinner = Spinner()
  ...
  # Alternate servers (Konik, custom) register their own dongle ID, which must never replace the stock one
  if not register_alternate and dongle_id not in (params.get("KonikDongleId"), params.get("CustomDongleId")):
